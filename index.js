...
//getMessagesClaude
async function getMessagesClaude(prompt, model = "claude-3-5-sonnet-20241022", temperature=0) {
  const messages = [{ role: "user", content: prompt }];
...
app.post('/v1/claude', async (req, res) => {
  ...
  const prompt = req.body.prompt; 
  // console.log(`prompt utente per Claude:  ${prompt}`);
  if (!prompt) {
    return res.status(400).send({ error: "Prompt is required" });
  }

  try {
    const completion = await getMessagesClaude(prompt); 
    console.log(completion);
    res.status(200).json({ completion });
   //res.status(200).json({answers:result.choices[0].message.content}); 
  } catch (error) {
